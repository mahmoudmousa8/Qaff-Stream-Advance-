    ...
    try {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const managerActiveSlots: number[] = data.activeStreams || []
          isManagerActive = managerActiveSlots.includes(slotIndex)
        }
      } catch {}
      console.log(`[Verification Diagnostic] Slot ${slotIndex + 1} (${action}): activeInManager=${isManagerActive}`)
    } catch (e: any) {
